...
bool CGameControllerCTF::OnEntity(int Index, vec2 Pos, int MapID)
{
	...
	CFlag *F = new CFlag(&GameServer()->m_World, Team, MapID);
	F->m_StandPos = Pos;
	F->m_Pos = Pos;
	m_apFlags[Team] = F;
...
void CGameControllerCTF::Tick()
{
	...
	for(int fi = 0; fi < 2; fi++)
	{
		...
		if(GameServer()->Collision(F->GetMapID())->GetCollisionAt(F->m_Pos.x, F->m_Pos.y)&CCollision::COLFLAG_DEATH || F->GameLayerClipped(F->m_Pos))
		{
		...
		else
		{
			...
			int Num = GameServer()->m_World.FindEntities(F->m_Pos, CFlag::ms_PhysSize, (CEntity**)apCloseCCharacters, MAX_CLIENTS, CGameWorld::ENTTYPE_CHARACTER, F->GetMapID());
			for(int i = 0; i < Num; i++)
			{
				if(!apCloseCCharacters[i]->IsAlive() || apCloseCCharacters[i]->GetPlayer()->GetTeam() == TEAM_SPECTATORS || GameServer()->Collision(F->GetMapID())->IntersectLine(F->m_Pos, apCloseCCharacters[i]->m_Pos, NULL, NULL))
					continue;
			...
			if(!F->m_pCarryingCharacter && !F->m_AtStand)
			{
				...
				else
				{
					...
					GameServer()->Collision(F->GetMapID())->MoveBox(&F->m_Pos, &F->m_Vel, vec2(F->ms_PhysSize, F->ms_PhysSize), 0.5f);
				}
			}
		}
	}
